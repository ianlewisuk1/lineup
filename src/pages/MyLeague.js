...
// NEW: Week selector component
  ...
  useEffect(() => {
      const fetchPlayoffBracket = async () => {
        // Inline getCurrentWeekNumber logic to avoid hoisting issues
        let currentWeekNum = 1;
        if (typeof currentWeek === 'number') {
          currentWeekNum = currentWeek;
        } else if (currentWeek && typeof currentWeek === 'string') {
          const weekMatch = currentWeek.match(/\d+/);
          currentWeekNum = weekMatch ? parseInt(weekMatch[0]) : 1;
        }
...
// Enhanced Team Card Modal - Updated to show current week game
  ...
  console.log('currentWeekNum:', currentWeekNum);
  console.log('playoffBracket exists:', !!playoffBracket);
  console.log('viewMode:', viewMode);
  console.log('Should show playoff bracket:', currentWeekNum >= 12 && playoffBracket && viewMode === 'current');
